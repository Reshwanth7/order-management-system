...
## Apr 20 2026 — Phase 1 Day 11 COMPLETE
**Built:** Dockerfile (multi-stage) for all services + docker-compose.yml wiring all services together including Zipkin
**Learned:** Multi-stage Dockerfile (JDK stage to compile, JRE stage to run — smaller final image), why localhost doesn't work inside Docker containers (each container has its own network namespace), Docker container names as DNS hostnames on a shared bridge network, docker-compose healthchecks and depends_on conditions (service_started vs service_healthy), why service_healthy is needed for Config Server and Eureka before dependent services start, Zipkin runs from official Docker Hub image (no Dockerfile needed), @Value for Zipkin endpoint instead of hardcoding URL so it works both locally and in Docker
**Bugs fixed:** jwt.secret not resolving in container (localhost:8888 in bootstrap.yml — changed to config-server:8888), Eureka registration failing (localhost:8761 in config files — changed to eureka-server:8761), api-gateway couldn't reach Eureka (localhost in application.yml — changed to eureka-server:8761), user-service starting before Config Server ready (changed depends_on condition from service_started to service_healthy), Zipkin traces not appearing in Docker (TracingConfig hardcoded localhost:9411 — changed to read endpoint from @Value), eureka-server healthcheck failing (missing spring-boot-starter-actuator — added to pom.xml)
**Tests passing:** docker compose up --build starts all services in correct order, POST /api/auth/register returns token, POST /api/auth/login returns token, POST /api/orders with valid token returns 201, GET /api/orders/{id} returns order, POST /api/orders with invalid userId returns 404, POST /api/orders without token returns 401, Zipkin at http://localhost:9411 shows traces spanning both services, Eureka dashboard at http://localhost:8761 shows all services registered
**Next:** Day 12 — PostgreSQL + replace H2 in Docker